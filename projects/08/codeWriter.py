...
def translate(parsed_fields, file_prefix):   # accepts array - ["push", "local", "3"], and "StaticTest" file prefix
    ...
    if command in ["eq", "gt", "lt", "add", "sub", "neg", "and", "or", "not"]:  # all 9 arithmetic/logical
        ...
        return get_push_pop_codes(parsed_fields, file_prefix)   # file_prefix provided for static variable names
    elif command in ["label", "if-goto", "goto"]:
        return get_branching_code(parsed_fields)
    else:
        return ["SOME OTHER INSTRUCTION NOT IMPLEMENTED"]
...
def get_push_pop_codes(parsed_fields, file_prefix):
    ...
    if (command == "push"):
        return get_push_codes(parsed_fields, file_prefix)
    else:
...
def get_label_code(parsed_fields):
    label = parsed_fields[1]

    return [f"({label})"]

def get_branching_code(parsed_fields):
    command = parsed_fields[0]
    label = parsed_fields[1]

    if command == "label":                      # label LABEL
        return get_label_code(parsed_fields)
    elif command == "goto":                     # goto LABEL
        asm_codes = [f"@{label}", "0;JMP"]
    elif command == "if-goto":                  # if-goto LABEL
        asm_codes = ["@SP", "M=M-1", "A=M", "D=M", f"@{label}", "D;JNE"]
